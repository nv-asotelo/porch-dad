...
**Deploy on another Jetson:** use the [agent deployment runbook](docs/agent-deployment.md), including its copyable agent handoff. It provides hardware/software gates, pinned source and model verification, on-device MLP engine construction, account/path configuration, service installation, optional HTTPS and fresh acceptance. Orin Nano 8 GB is the tested reference; Orin NX and AGX Orin are SM87 candidates requiring their own validation. This release does not claim compatibility with every Jetson or include prebuilt model artifacts. Device-local configuration and TLS files stay ignored by Git.
...
The original native build completed both required targets and all 25 selected SM87 FMHA variants in 45m44s. Its [build receipt](results/native-backend-build.json) describes compilation, separately from inference. For your own deployment, follow [Connect to your Jetson](#connect-to-your-jetson). Device addresses and account names in those instructions are placeholders; this repository does not provide a public demo endpoint. The selected `data/engine-cache-mlp-compact` engine quantizes 56 MLP weights to RTN INT4 and keeps attention, vision, embeddings, activations and KV in FP16. The recorded benchmark configuration uses dynamic clocks within stock 25 W, input capacity 1,024, KV capacity 1,664, aggregate/per-image visual capacities 512/512, batch one and zero encoder-cache budget. The latest native extension supplies the retained controls and timing instrumentation. Swap remains disabled, with its original file and configuration backup preserved.
...
For a new device or a different account/directory, start with the [portable agent runbook](docs/agent-deployment.md). The instructions and receipts below also retain the original deployment's history. The service helpers now accept `--user` and a generated `deployment/local.env`; the older source-transfer helper still intentionally uses its original fixed destination.
...
3. Follow [backend build and checkpoint instructions](docs/backend-build.md) to prepare the verified model and pinned backend in your project directory. Preserve the checkpoint's original files and notices. Stop your model service before rebuilding or loading another candidate.
4. The recorded deployment uses systemd services. After installing them for your own device and account:
...
The selected launcher validates and loads the existing engine cache. Rebuild explicitly using [backend instructions](docs/backend-build.md) when the model, TensorRT version or engine profile changes. Check `http://127.0.0.1:8090/health/ready` on the Jetson before inference. The UI shows backend unavailability while the model loads. Deployment helpers retain assumptions from the original device; review their account, project-directory and listener settings before installing them on another machine.

### Connect to your Jetson

**Use SSH forwarding for personal access.** Replace `YOUR_JETSON_USER` and `YOUR_JETSON_ADDRESS` with your own account and device address, and verify the SSH host fingerprint on first connection:

```bash
ssh -N -L 127.0.0.1:8090:127.0.0.1:8090 YOUR_JETSON_USER@YOUR_JETSON_ADDRESS
```

Then open [http://localhost:8090](http://localhost:8090). The browser treats localhost as a secure camera context; SSH protects the connection to the Jetson. This explicit local bind keeps the forwarded port on your computer's loopback interface. SSH forwarding does not close LAN listeners that were enabled separately on the Jetson.

**Optional direct LAN access:** configure HTTPS for your device and visit `https://YOUR_JETSON_ADDRESS:8443`. The [LAN helper](scripts/enable_lan_ui.sh) is specific to the recorded deployment: it enables an HTTP listener on port 8090 and an HTTPS listener on port 8443 on all interfaces. The page redirects to HTTPS, but API and health routes also remain available over HTTP. The native backend stays on loopback port 8000.

**This demo has no application authentication.** HTTPS encrypts traffic but does not restrict who can use the inference API. Keep the service on a trusted network with appropriate firewall restrictions; do not port-forward it onto the Internet. For broader access, put authentication and TLS termination in front of it and restrict the direct listeners. The SSH example above authenticates the tunnel, not independently enabled LAN access.

Use a certificate issued by a trusted authority, or verify your device-local certificate's SHA-256 fingerprint through a trusted channel before accepting it in the browser. From the project directory on your Jetson:

```bash
openssl x509 -in deployment/tls/orin.crt -noout -fingerprint -sha256
```

Compare that fingerprint with the certificate shown by the browser before granting camera permission. Keep private keys and credentials out of Git. See [UI deployment notes](research/ui-feasibility.md) for the implementation and its recorded tests.
