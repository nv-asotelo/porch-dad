...
The resource cards below the video show colored **sparklines for the last 60 seconds**, with per-chart scales and gaps for missing readings. **Latency** is the latest successful capture-to-complete-answer duration, including JPEG encoding, in milliseconds. **Average** is the cumulative arithmetic mean of completed requests, matching Live VLM WebUI's averaging rule. Failed and canceled requests are excluded. Reloading or changing capture preset resets the count and average; pausing or stopping preserves them. The existing first-token and request-to-stream-end measurements remain under **Timing details**. See [caption, sparkline and latency definitions](research/ui-caption-sparklines.md).
