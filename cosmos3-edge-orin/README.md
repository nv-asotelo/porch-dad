...
**Deployment status: JetPack is installed via USB-C. The final selected deployment returns to MLP-only RTN INT4 with the original lightweight demo parameters, at the user's request after a temporary FP16 restoration.** The preserved compact MLP engine, later UI controls and server timing remain available. The original FP16 model/cache and its historical evidence are preserved separately. This selection alone does not establish a measured speed advantage; matched TTFT results require their own receipts. Backend and UI are managed by systemd services. USB-C flash, normal NVMe boot, and a CUDA GPU calculation passed. The Orin Nano 8 GB developer kit (P3767-0005) runs Ubuntu 24.04.4 / Jetson Linux 39.2.1 with CUDA compiler 13.2.86 and TensorRT 10.16.2.10. Its WD_BLACK SN7100 1 TB NVMe root partition and ext4 filesystem were expanded successfully. Source/checkpoint transfer and Python dependency checks passed. The pinned reasoner snapshot contains 21 digest-verified files totaling 7,735,525,563 bytes.
...
The resource cards show colored **sparklines for the last 60 seconds**, with per-chart scales and gaps for missing readings. In closed-caption modes they fill the right column beneath output info. **Latency** now measures native inference on the Orin, excluding JPEG encoding/decoding, queueing and network transport. **Average** is the cumulative mean of successful server-timed requests in the current settings group. Browser first-text, round-trip and frame-age diagnostics remain separate. Historical browser-timed results are unchanged.
