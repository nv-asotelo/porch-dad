...
The user originally requested a toggle for comparison with Live VLM WebUI and made that the default. After the later runtime-control experiment, the user restored the **first demo's Lightweight defaults**, tried the original FP16 engine, then explicitly selected **MLP INT4 again while keeping those lightweight parameters**. The final page starts in Lightweight mode with the original prompt and 64-token output limit on the preserved compact MLP engine. The original FP16 cache, interim MLP acceptance and temporary FP16 TTFT records remain separate. Native server timing, advanced image-token controls and the Live VLM capture option remain available. Switching capture presets keeps the same loaded model; it is not an implementation or full-stack performance equivalence claim.

| Setting | Live VLM WebUI capture preset | Lightweight preset (default) |
| --- | --- | --- |
...
| top_p / top_k | Advanced setting defaults to 1.0; top_k=50 | Greedy decoding resolves 1 / 1 |
| Prompt when toggling | Preserves the current editable prompt | Preserves the current editable prompt |
| Output cap when toggling | Preserves the current editable cap | Preserves the current editable cap |

Every new page starts with Lightweight capture, the prompt “Describe the visible scene in one concise sentence. Focus on objects and actions.” and a 64-token output limit. For the pinned upstream prompt/output settings, choose Scene Description in Quick presets (“Describe what you see in this image in one sentence.”) and set the output limit to 512. Choosing the Live VLM capture preset does not overwrite those fields. Switching stops camera tracks, aborts the active request, cancels frame callbacks, clears output/timings/counters and prevents late stream cleanup from overwriting the new mode. Prompt and token edits are retained across the toggle so the source workload can be held constant. The camera label shows actual dimensions/FPS from the negotiated track, and the capture label shows actual JPEG dimensions. Preset values are requests, not a promise that every physical camera supplies 1280×720.

The selected load policy is now an image cap of 512, top-p 1.0, zero encoder-cache budget and dynamic clocks within stock 25 W. The same-day 320-token / top-p 0.95 / 256 MiB cache / static-clock acceptance remains historical evidence. Its subsequent request-log comparison was interrupted by the restoration request; no completed fixed/marginal latency comparison is claimed.
...
- This UI retains token streaming, while upstream requests a non-streaming answer. Compare complete-answer latency with the same source, model, prompt and token limit; first-token timings are not an upstream equivalent. The headline latency now surrounds native inference on the server, excluding JPEG encoding/decoding and network transfer. Upstream browser/API timers have different boundaries and cannot be compared directly; use equivalent server request logs.
- The model applies native image preprocessing under a per-request cap: 512 by default, 320 or a custom value within the loaded engine. The cap is not the actual visual-token count, the camera resolution or the output token limit. Runtime measurements supersede earlier formula estimates; see [server-side measurement](runtime-controls-server-timing.md).
- All existing four-trial optimization, quality grading and benchmark records stay unchanged. The optional Live VLM preset's 0.7-temperature setting is stochastic, so exact prior outputs and timings should not be expected. The final selection returns to compact MLP INT4 with the latest native controls and server-only timing; historical client timings remain measurements of their original boundary. The temporary FP16 restoration and final MLP choice do not themselves establish a measured speed difference. No new optimization search is implied by either preset.
...
The 2026-09-21 UI update copies the ten Quick Presets labels and prompt texts exactly from the same public Live VLM WebUI revision, which was also its verified public HEAD on that date. They are Scene Description, Object Detection, Activity Recognition, Safety Monitoring, Emotion Detection, Accessibility, OCR / Text Reading, Yes/No Question, Robot Navigation (Simple) and Robot Navigation (ROS). Selecting one fills the editable prompt. Editing it to a different value selects Custom prompt. The restored initial concise-scene prompt differs from these upstream presets and therefore uses Custom prompt; its output limit starts at 64. The robot examples generate text only; this interface does not execute commands or control a robot.

The explicit caption selector offers Side window, Above camera and Below camera. Below camera is the first-visit default. The browser remembers a valid selection, falling back to Below if storage is blocked or invalid. Side window stacks below the camera on screens at most 800 CSS pixels wide. Following the user's recording reference, Above and Below now use a compact black band attached to the video. The implementation moves only the existing caption element between docks, preserving the video, active request, streamed text and metrics. Upstream has separate vertical-order and on-camera-overlay controls, plus automatic side-by-side layout on wide screens. This task's three-position selector implements the requested placement without covering the video. [Current caption, sparkline and latency behavior](ui-caption-sparklines.md) supersedes the earlier whole-panel layout.
...
The historical [direct Orin browser verification](../results/ui-controls-browser.json) passed actual upload inference, incremental text, all caption positions, manual camera capture, live pause/resume, prompt submission and mobile layout under the then-current defaults. Camera frames were Chromium's synthetic feed. Its [deployment verification](../results/ui-controls-deployment.json) checks that revision's served asset hashes and selected runtime. The existing SSE tests also cover manual/automatic request ownership and caption movement during a stream. These are functional checks, not a new latency or accuracy experiment or a receipt for the restored defaults.
